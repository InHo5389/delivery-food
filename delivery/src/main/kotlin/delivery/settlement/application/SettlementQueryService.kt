...
class SettlementQueryService(
    ...
    // 형식 검증(연월 파싱)을 소유권 조회보다 먼저 해서, 요청 자체가 잘못됐을 땐 굳이
    ...
    fun getMySettlement(requester: AuthenticatedUser, query: MySettlementQuery): SettlementSummaryResult {
        val yearMonth = parseYearMonth(query.yearMonth)
    ...
    fun getAdminSettlements(requester: AuthenticatedUser, query: AdminSettlementRangeQuery): List<SettlementSummaryResult> {
        ...
        val (start, _) = monthRange(parseYearMonth(query.from))
        val (_, end) = monthRange(parseYearMonth(query.to))
    ...
    private fun toSummary(settlement: Settlement): SettlementSummaryResult {
        ...
        return SettlementSummaryResult(
            ...
            period = YearMonth.from(settlement.periodStart.atZone(KST)),
    ...
    private fun parseYearMonth(value: String): YearMonth =
        try {
            ...
            throw BusinessException(SettlementErrorCode.INVALID_YEAR_MONTH)
        }
}
